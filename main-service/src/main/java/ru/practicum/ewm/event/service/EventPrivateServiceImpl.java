...
public class EventPrivateServiceImpl implements EventPrivateService {
    ...
    public EventFullDto add(NewEventDto newEventDto, Long userId) {
        ...
        if (event.getEventDate().isBefore(LocalDateTime.now().plusHours(2))) {
            throw new ConflictException("Дата события не раньше чем через 2 часа.");
        }
    ...
    public EventFullDto update(Long userId, Long eventId, UpdateEventUserRequest updateEvent) {
        ...
        if (!event.getEventDate().isBefore(LocalDateTime.now().plusHours(2))) {
            ...
            if (event.getState().equals(State.CANCELED) || event.getState().equals(State.PENDING)) {
                ...
                if (updateEvent.getStateAction().equals(PrivateUpdateStatus.SEND_TO_REVIEW)) event.setState(State.PENDING);
                if (updateEvent.getStateAction().equals(PrivateUpdateStatus.CANCEL_REVIEW))  event.setState(State.CANCELED);
                ...
                log.info("Update EventStatus -> {}, userId={}", updateEvent.getStateAction(), userId);
    ...
    public EventRequestStatusUpdateResult updateStatus(Long userId, Long eventId,
        ...
        RequestStatus status = statusUpdateRequest.getStatus();
        ...
        if (event.getParticipantLimit().equals(event.getConfirmedRequests())) {
            ...
            throw new ConflictException("Невозможно подтвердить заявку, достигнут лимит.");
        }
